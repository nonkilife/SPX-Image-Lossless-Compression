...
def extract_channels(rgb: npt.NDArray[np.uint8]) -> Tuple[npt.NDArray[np.uint8], npt.NDArray[np.uint8], npt.NDArray[np.uint8], npt.NDArray[np.uint8], npt.NDArray[np.uint32]]:
    ...
    a_map = np.empty((h, w), dtype=np.uint8) if c == 4 else np.empty((1, 1), dtype=np.uint8)
    ...
    for i in prange(h):
        for j in range(w):
            ...
            rd_map[i, j] = rd_v
            bd_map[i, j] = bd_v
            row_hists[i, 0, g] += 1; row_hists[i, 1, rd_v] += 1; row_hists[i, 2, bd_v] += 1
            if c == 4: a_map[i, j] = rgb[i, j, 3]
            
    global_hists = np.zeros((3, 256), dtype=np.uint32)
    for i in range(h):
        global_hists += row_hists[i]
...
@njit(boundscheck=True, error_model='numpy', cache=True)
def decode_alpha_channel(h: int, w: int, res_ch: npt.NDArray[np.uint8], rec_ch: npt.NDArray[np.uint8]) -> None:
    ...
    for i in range(h):
        ...
        # Use row 0 as a type-consistent placeholder when i==0; it is never read
        # for prediction in that case (all guards check i > 0).
        row_trec = rec_ch[i-1] if i > 0 else rec_ch[0]
        a_val, c_val = np.uint8(0), np.uint8(0)
        for j in range(w):
            b_val = row_trec[j] if i > 0 else np.uint8(0)
            p = predict_med_standard(a_val, b_val, c_val if j > 0 else np.uint8(0))
            val = np.uint8((from_zigzag(res_row[j]) + int(p)) & 0xFF)
            row_rec[j] = val
            a_val, c_val = val, (row_trec[j] if i > 0 else np.uint8(0))
